...
from evaluation.utils.habitat_extensions.config.default import get_extended_config as get_task_config

# ----------------------------------------------------------------------------
# EXPERIMENT CONFIG
# ----------------------------------------------------------------------------
_C = CN()
_C.BASE_TASK_CONFIG_PATH = "evaluation/utils/habitat_extensions/config/vlnce_task.yaml"
...
def get_config(
    ...
    """
    config = CN()       # CfgNode
    config.merge_from_other_cfg(habitat_baselines.config.default._C)    # merge Habitat-Baselines 的“全套默认配置”
    purge_keys(config, ["SIMULATOR_GPU_ID", "TEST_EPISODE_COUNT"])      # 清理不需要的字段
    config.merge_from_other_cfg(_C.clone()) # merge VLN-CE / 项目自己的默认配置, _C是你项目里定义的默认配置（不是 Habitat 的）
    ...
    if config_paths:
        ...
        for config_path in config_paths:
            ...
            # BASE_TASK_CONFIG_PATH 指向的 YAML
            # 被加载成一个新的 CfgNode，
            # 挂在 config.TASK_CONFIG 下面
